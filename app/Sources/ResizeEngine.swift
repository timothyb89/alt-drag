...
// --- AX helpers ------------------------------------------------------------
private func axPoint(_ e: AXUIElement, _ attr: String) -> CGPoint? {
    var v: CFTypeRef?
...
private func axSize(_ e: AXUIElement, _ attr: String) -> CGSize? {
    var v: CFTypeRef?
    guard AXUIElementCopyAttributeValue(e, attr as CFString, &v) == .success else { return nil }
    var s = CGSize.zero
...
private func setAXPoint(_ e: AXUIElement, _ attr: String, _ p: CGPoint) {
    var p = p
    if let v = AXValueCreate(.cgPoint, &p) { AXUIElementSetAttributeValue(e, attr as CFString, v) }
}
private func setAXSize(_ e: AXUIElement, _ attr: String, _ s: CGSize) {
    var s = s
    if let v = AXValueCreate(.cgSize, &s) { AXUIElementSetAttributeValue(e, attr as CFString, v) }
}
private func windowUnder(_ cursor: CGPoint) -> AXUIElement? {
